...
8. Откройте файл `Dockerfile` в корне `liaksej-tiny-cloud-frontend`. На 26 строке замените `http://localhost/` на
    имя вашего хоста.
   ```dockerfile
   # ...
   # Замените http://localhost/ на имя вашего хоста.
...
2. Находясь в каталоге `liaksej-tiny-cloud-backen`, запустите сборку приложения через командную строку:
   ...
   ```
Примечания:
> Если при сборке контейнера nextjs возникнут сложности с buildx и вы разбираетесь в Docker, можете установить buildx 
> по [инструкции](https://github.com/docker/buildx#linux-packages). Если нет времени разбираться, замените
> в Dockerfile, расположенном в директории `liaksej-tiny-cloud-frontend` строки 16 и 33:
> ```dockerfile
> # Заменить строку 16
...
> ```
> Собираться будет дольше. Но результат тот же. 
> 
...
1. После сборки проекта выполните следующие команды для применения миграций, сборке статики для Django и созданию суперпользователя:
   ...
   > 
   > Для администрирования пользовательского интерфейса пройдите регистрацию в приложении `http://your.host/signup`, а затем через 
   > первого суперпользователя выдайте права администратора зарегистрированному пользователю в административной панели Django 
   > `http://your.host`. Таким образом зарегистрированный и получивший права администратора пользователь сможет выполнять все действия
